  ...
  try {
    ...
    if (user?.splitwise?.interests && id) {
      const interest = user.splitwise.interests.find(
        (interest: { friend_id: number; settings: Settings }) => interest.friend_id === id,
      );
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const user = await User.findOne({ ["username"]: session.user?.name });
    if (!user) {
      return NextResponse.json({ message: "Unauthorized: Cant find User" }, { status: 401 });
    }
    ...
    // Get Data from User
    if (user?.splitwise?.interests && id) {
      let interest = user.splitwise.interests.find(
        (i: { friend_id: number; settings: Settings }) => i.friend_id === id,
      );

      if (!interest) {
        user.splitwise.interests.push({
          friend_id: Number.parseInt(params.id),
          settings: {
            apy: 0,
            cycles: 0,
            minDebtAge: 0,
